...
class IdPortalClient
{
    ...
    public function appsFor(User $user): array
    {
        ...
        if (! is_array($cached)) {
            ...
            Cache::put($key, $cached, Config::integer('services.id.portal_cache_ttl', 300));
        }
        ...
        $currentSlug = Config::string('services.id.slug');
    ...
    private function token(): ?string
    {
        ...
        $response = Http::acceptJson()->asForm()->post($this->url('/oauth/token'), [
            ...
            'client_id' => config('services.id.client_id'),
            'client_secret' => config('services.id.client_secret'),
        ]);
    ...
    {
        return filled(config('services.id.base_url'))
            && filled(config('services.id.client_id'))
            && filled(config('services.id.client_secret'));
    }

    private function url(string $path): string
    {
        return rtrim(Config::string('services.id.base_url'), '/').$path;
    }
}
